...
char * base64::base64Encode(const unsigned char *message, const size_t length) {
    BIO *bio;
    BIO *b64;
    FILE* stream;
    ...
    if(!buffer) {qDebug() << "bad memory base64 encode"; exit(1);}
...
int base64::base64Decode(const char *b64message, const size_t length, unsigned char **buffer) {
    BIO *bio;
    BIO *b64;
    int decodedLength = calcDecodeLength(b64message, length);
    *buffer = (unsigned char*)malloc(decodedLength+1);
    if(!(*buffer)) { qDebug() << "bad memory base64 Decode"; exit(1);}
...
int base64::calcDecodeLength(const char *b64input, const size_t length) {
    int padding = 0;
